...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, H=None, W=None) -> Tensor:
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            ...
            x = x + attn_residual_func(x).to(x.dtype)
            x = x + ffn_residual_func(x).to(x.dtype)
        return x
...
class NestedTensorBlock(Block):
    ...
    def forward(self, x_or_x_list, H=None, W=None):
